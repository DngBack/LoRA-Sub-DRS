...
def compute_diagonal_fim(model, dataloader, device):
    ...
    for batch_idx, (_, images, labels) in enumerate(dataloader):
        ...
        # Check for dimension mismatch and fix labels
        num_classes = logits.size(1)

        # Ensure labels are within valid range [0, num_classes-1]
        # Clamp labels to prevent index out of bounds
        labels_clamped = torch.clamp(labels, 0, num_classes - 1)

        # Check if any labels were out of bounds
        if torch.any(labels >= num_classes):
            print(
                f"Warning: Found labels >= num_classes ({num_classes}). Max label: {labels.max().item()}"
            )
            print(f"Clamping labels to valid range [0, {num_classes - 1}]")

        # Use log_softmax to calculate log-likelihood
        log_probs = F.log_softmax(logits, dim=1)

        # Get log-likelihood of the correct class (using clamped labels)
        sampled_log_probs = log_probs.gather(1, labels_clamped.view(-1, 1))
        ...
        try:
            ...
            print(f"sampled_log_probs shape: {sampled_log_probs.shape}")
        ...
        for name, p in model.named_parameters():
            if p.grad is not None and name in fim:
                ...
                if torch.isfinite(p.grad).all():
                    ...
                    print(f"Warning: Non-finite gradients detected in parameter {name}")
                    # Skip this parameter for this batch
    ...
    if total_samples > 0:
        ...
        print("Warning: No samples processed for FIM computation")
...
def compute_lambda_star(
    theta_t_minus_1, theta_t_cand, current_fim, accumulated_fim, epsilon=1e-8
):
    """
    ...
    delta_theta = {
        name: theta_t_cand[name] - theta_t_minus_1[name]
        for name in theta_t_cand
        if name in theta_t_minus_1
    }

    # Calculate terms for the lambda* formula
    # Get device from one of the parameters
    device = next(iter(current_fim.values())).device if current_fim else "cpu"

    numerator = torch.tensor(0.0, device=device)
    denom_term1 = torch.tensor(0.0, device=device)
    denom_term2 = torch.tensor(0.0, device=device)
    ...
    denominator = denom_term1 + denom_term2 + epsilon

    # Ensure we don't divide by zero
    if denominator > 0:
        lambda_star = numerator / denominator
    else:
        lambda_star = torch.tensor(0.5, device=device)  # Default fallback value
...
def adaptive_merge_parameters(theta_t_minus_1, theta_t_cand, lambda_star):
    ...
    for name in theta_t_cand:
        if name in theta_t_minus_1:
            ...
            merged_theta[name] = (1 - lambda_star) * theta_t_minus_1[
                name
            ] + lambda_star * theta_t_cand[name]
        else:
