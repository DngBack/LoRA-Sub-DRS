...
class ADDRSAnalyzer:
    ...
    def __init__(self, experiment_name, save_dir="experiments"):
        ...
        self.experiment_dir = os.path.join(
            save_dir, f"{experiment_name}_{self.timestamp}"
        )
    ...
    def log_config(self, config):
        ...
        self.results["config"] = serializable_config
    ...
    def _make_serializable(self, obj):
        ...
        if isinstance(obj, dict):
            ...
            # For basic types (int, float, str, bool, None)
            try:
    ...
    def log_task_results(self, task_id, task_acc, avg_acc, task_time=None):
        ...
        logging.info(
            f"Task {task_id} completed: Acc = {task_acc:.2f}%, Avg = {avg_acc:.2f}%"
        )
    ...
    def compute_forgetting_measure(self, all_task_accs):
        ...
        logging.info(
            f"Average forgetting after task {len(all_task_accs) - 1}: {avg_forgetting:.2f}%"
        )
    ...
    def _plot_accuracy_evolution(self):
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(self.experiment_dir, "accuracy_evolution.png"), dpi=300
        )
        plt.close()
    ...
    def _plot_forgetting_analysis(self):
        ...
        avg_forgetting = [
            item["average_forgetting"] for item in self.results["forgetting_measures"]
        ]
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(self.experiment_dir, "forgetting_analysis.png"), dpi=300
        )
        plt.close()
    ...
    def _print_summary(self):
        ...
        if self.results["forgetting_measures"]:
            final_forgetting = self.results["forgetting_measures"][-1][
                "average_forgetting"
            ]
