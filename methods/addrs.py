...
from utils.refinement import ClassifierAlignment, SelfDistillationLoss, apply_self_distillation
...
class AD_DRS(LoRAsub_DRS):
    ...
    def __init__(self, args):
        ...
        if self.use_classifier_alignment:
            self.classifier_aligner = ClassifierAlignment(feature_dim=args.get("embd_dim", 768))
        ...
        experiment_name = f"AD-DRS_{args.get('dataset', 'unknown')}_{args.get('prefix', 'exp')}"
        self.analyzer = ADDRSAnalyzer(experiment_name)
        self.analyzer.log_config(args)
    ...
    def after_task(self):
        ...
        avg_acc = sum(self.lambda_history) / len(self.lambda_history) * 100 if self.lambda_history else task_acc
        ...
        if hasattr(self, 'fisher_manager') and self.fisher_manager.accumulated_fisher:
            fisher_norm = sum(torch.norm(fim).item() for fim in self.fisher_manager.accumulated_fisher.values())
        ...
        if self.use_classifier_alignment and hasattr(self, 'train_loader'):
            ...
            self.classifier_aligner.update_class_stats(
                self._network, self.train_loader, self._device, 
